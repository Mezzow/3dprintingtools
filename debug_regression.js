// Regression test: DejaVu Sans with flood fill approach
...
// Flood fill connection detection
...
function floodFillFrom(seedX, seedY) {
  ...
  while (stack.length > 0) {
    var sy = stack.pop(); var sx = stack.pop();
...
for (var ci = 1; ci < text.length; ci++) {
  ...
  var isConnected = rightSeed ? reachable[rightSeed[1] * cw + rightSeed[0]] === 1 : false;

  if (isConnected) {
    console.log("'" + text[ci-1] + "|" + text[ci] + "' -> CONNECTED, skip");
    continue;
  }
  if (rightSeed) {
    var newReach = floodFillFrom(rightSeed[0], rightSeed[1]);
    for (var pi = 0; pi < newReach.length; pi++) { if (newReach[pi]) reachable[pi] = 1; }
  }
  ...
  if (bwidth > 0) {
    ...
    console.log("'" + text[ci-1] + "|" + text[ci] + "' -> BRIDGE (" + bwidth + "px) ink=" + leftInk + ".." + rightInk);
  }
}
...
fs.writeFileSync("debug_regression.png", cv.toBuffer("image/png"));
console.log("Saved debug_regression.png");
